...
3. `kimi_cli/skills/mock_websites/SKILL.md` when `surface.yaml` or `query.md`
...
strings from query packages into final task metadata; AWS teacher runtime treats them as
...
4. Copy optional source manifests into the bundle: `task.yaml`, `spec.yaml`,
   ...
   AWS teacher runtime cannot run the placeholder instead of the real task.
